# =========================================================
# N-BEATS CO2 Forecasting with 80% & 95% Prediction Intervals
# =========================================================
...
# N-BEATS Block
# ---------------------------
def nbeats_block(input_dim, theta_dim, hidden_units=128):
    x = layers.Dense(hidden_units, activation='relu')(input_dim)
    x = layers.Dense(hidden_units, activation='relu')(x)
    x = layers.Dense(hidden_units, activation='relu')(x)

    theta = layers.Dense(theta_dim, activation='linear')(x)

    backcast = layers.Dense(n_lags, activation='linear')(theta)
    forecast = layers.Dense(1, activation='linear')(theta)
...
for i, country in enumerate(countries):
    ...
    # ---------------------------
    scaler = MinMaxScaler()
    scaled = scaler.fit_transform(ts[['CO2']]).flatten()
    ...
    # N-BEATS Model
    # ---------------------------
    inputs = layers.Input(shape=(n_lags,))
    residuals = inputs
    forecasts = []

    for _ in range(3):  # stacks
        backcast, forecast = nbeats_block(residuals, theta_dim=32)
    ...
    for _ in range(n_bootstrap):
        ...
        boot_preds_test.append(y_pred.flatten() + noise)
    ...
    # ---------------------------
    last_input = scaled[-n_lags:]
    future_scaled = []

    for _ in range(n_future):
        next_val = model.predict(last_input.reshape(1, -1))[0][0]
...
plt.title("N-BEATS CO₂ Forecasting with 80% & 95% Prediction Intervals")
plt.legend()
plt.grid(True)
plt.tight_layout()
plt.show()

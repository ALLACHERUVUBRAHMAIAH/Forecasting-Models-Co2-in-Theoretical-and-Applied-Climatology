# =========================================================
# Decision Tree CO2 Forecasting with 80% & 95% Prediction Intervals
# =========================================================
...
for i, country in enumerate(countries):
    ...
    # ---------------------------
    scaler = MinMaxScaler()
    scaled_co2 = scaler.fit_transform(ts[['CO2']]).flatten()
    ...
    # ---------------------------
    future_input = scaled_co2[-n_lags:].reshape(1, -1)
    future_scaled = []

    for _ in range(n_future):
        next_val = model.predict(future_input)[0]
...
plt.title("Decision Tree CO₂ Forecasting with 80% & 95% Prediction Intervals")
plt.legend()
plt.grid(True)
plt.tight_layout()
plt.show()

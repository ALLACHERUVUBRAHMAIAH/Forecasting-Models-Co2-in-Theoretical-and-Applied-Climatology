# =========================================================
# Random Forest CO2 Forecasting with 80% & 95% Prediction Intervals
# =========================================================
...
for i, country in enumerate(countries):
    ...
    for j in range(n_lags, len(scaled_co2)):
        X.append(scaled_co2[j - n_lags:j].flatten())  # Flatten for RF
        y.append(scaled_co2[j])

    X = np.array(X)
    y = np.array(y).flatten()  # RF expects 1D target
    ...
    # Training Residuals
    # ---------------------------
    train_pred_scaled = rf_model.predict(X_train)
    ...
    # ---------------------------
    future_scaled = []
    last_window = scaled_co2[-n_lags:].flatten()

    for _ in range(n_future):
        next_val = rf_model.predict(last_window.reshape(1, -1))[0]
        future_scaled.append(next_val)
        last_window = np.append(last_window[1:], next_val)  # Slide window
...
plt.title("Random Forest CO₂ Forecasting with 80% & 95% Prediction Intervals")
plt.legend()
plt.grid(True)
plt.tight_layout()
plt.show()
